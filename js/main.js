...
if (/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent)) {
      ...
      for(n = 0 ; n < playerSegment.cars.length ; n++) {
        ...
        if (speed > car.speed) {
          if (Util.overlap(playerX, playerW, car.offset, carW, 0.8)) {
             Dom.get('last_lap_time').textContent = 'kaza yaptın arabaya çarptın';
             if (!motorkoruma) {
              fast_lap_time--;
             }
           doModal('kaza');
    ...
    var ROAD = {
      LENGTH: { NONE: 0, SHORT:  99, MEDIUM:   99, LONG:  100 },
      HILL:   { NONE: 0, LOW:    0, MEDIUM:   0, HIGH:   0 },
      CURVE:  { NONE: 0, EASY:    0, MEDIUM:    0, HARD:    0 }
    };
    ...
    function resetSprites() {
      ...
      for(n = 10 ; n < 200 ; n += 4 + Math.floor(n/100)) {
        addSprite(n, SPRITES.PALM_TREE, -2 + Math.random()*0.5);
